...
@ManagedBean
@ApplicationScoped 
public class SnapshotVideoFileProcessor implements MediaFileProcessor {
	...
	protected final MediaConfiguration config;
	
	protected final BufferedImage watermarkImage;
	
	private final RequestScopeCache<File, BufferedImage> imageCache;
	
	@Inject
	public SnapshotVideoFileProcessor(@NonNull MediaConfiguration config, @NonNull RequestScopeCache<File, BufferedImage> imageCache) {
		this.config = config;
	...
	public void createPreview(@NonNull File sourceFile, @NonNull File outputFile) throws IOException {
		Thumbnails
			.of(imageCache.memoized(sourceFile, this::takeSnapshot))
